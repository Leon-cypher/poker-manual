...
export type DocumentNode = Manual | Folder;
...
export type Requester<C= {}> = <R, V>(doc: DocumentNode, vars?: V, options?: C) => Promise<R>
  export function getSdk<C>(requester: Requester<C>) {
    return {
      manual(variables: ManualQueryVariables, options?: C): Promise<{data: ManualQuery, errors?: { message: string, locations: { line: number, column: number }[], path: string[] }[], variables: ManualQueryVariables, query: string}> {
        return requester<{data: ManualQuery, errors?: { message: string, locations: { line: number, column: number }[], path: string[] }[], variables: ManualQueryVariables, query: string}, ManualQueryVariables>(ManualDocument, variables, options);
      },
    manualConnection(variables?: ManualConnectionQueryVariables, options?: C): Promise<{data: ManualConnectionQuery, errors?: { message: string, locations: { line: number, column: number }[], path: string[] }[], variables: ManualConnectionQueryVariables, query: string}> {
        return requester<{data: ManualConnectionQuery, errors?: { message: string, locations: { line: number, column: number }[], path: string[] }[], variables: ManualConnectionQueryVariables, query: string}, ManualConnectionQueryVariables>(ManualConnectionDocument, variables, options);
      }
    };
  }
...
export const ExperimentalGetTinaClient = () =>
  getSdk(
    generateRequester(
      createClient({
        url: "https://content.tinajs.io/2.1/content/90c014e0-1b7a-426f-a5a4-122106c653b8/github/main",
        queries,
      })
    )
  )
